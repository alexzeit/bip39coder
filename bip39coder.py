...
"fly","foam","focus","fog","foil","fold","follow","food","foot","force","forest","forget","fork","fortune",
...
"gallery","game","gap","garage","garbage","garden","garlic","garment","gas","gasp","gate","gather","gauge","gaze",
"general","genius","genre","gentle","genuine","gesture","ghost","giant","gift","giggle","ginger","giraffe","girl",
"give","glad","glance","glare","glass","glide","glimpse","globe","gloom","glory","glove","glow","glue","goat","goddess",
"gold","good","goose","gorilla","gospel","gossip","govern","gown","grab","grace","grain","grant","grape","grass",
...
"harvest","hat","have","hawk","hazard","head","health","heart","heavy","hedgehog","height","hello","helmet","help",
"hen","hero","hidden","high","hill","hint","hip","hire","history","hobby","hockey","hold","hole","holiday","hollow",
"home","honey","hood","hope","horn","horror","horse","hospital","host","hotel","hour","hover","hub","huge","human",
...
"income","increase","index","indicate","indoor","industry","infant","inflict","inform","inhale","inherit","initial","inject",
"injury","inmate","inner","innocent","input","inquiry","insane","insect","inside","inspire","install","intact","interest","into",
...
"maid","mail","main","major","make","mammal","man","manage","mandate","mango","mansion","manual","maple","marble",
"march","margin","marine","market","marriage","mask","mass","master","match","material","math","matrix","matter","maximum",
"maze","meadow","mean","measure","meat","mechanic","medal","media","melody","melt","member","memory","mention","menu",
...
"mom","moment","monitor","monkey","monster","month","moon","moral","more","morning","mosquito","mother","motion","motor",
...
"neglect","neither","nephew","nerve","nest","net","network","neutral","never","news","next","nice","night","noble",
"noise","nominee","noodle","normal","north","nose","notable","note","nothing","notice","novel","now","nuclear",
"number","nurse","nut","oak","obey","object","oblige","obscure","observe","obtain","obvious","occur","ocean",
"october","odor","off","offer","office","often","oil","okay","old","olive","olympic","omit","once","one","onion",
"online","only","open","opera","opinion","oppose","option","orange","orbit","orchard","order","ordinary","organ",
"orient","original","orphan","ostrich","other","outdoor","outer","output","outside","oval","oven","over","own",
"owner","oxygen","oyster","ozone","pact","paddle","page","pair","palace","palm","panda","panel","panic",
"panther","paper","parade","parent","park","parrot","party","pass","patch","path","patient","patrol","pattern",
"pause","pave","payment","peace","peanut","pear","peasant","pelican","pen","penalty","pencil","people","pepper",
"perfect","permit","person","pet","phone","photo","phrase","physical","piano","picnic","picture","piece","pig",
"pigeon","pill","pilot","pink","pioneer","pipe","pistol","pitch","pizza","place","planet","plastic","plate",
...
"punch","pupil","puppy","purchase","purity","purpose","purse","push","put","puzzle","pyramid","quality","quantum",
...
"resemble","resist","resource","response","result","retire","retreat","return","reunion","reveal","review","reward",
...
"spirit","split","spoil","sponsor","spoon","sport","spot","spray","spread","spring","spy","square","squeeze",
...
"such","sudden","suffer","sugar","suggest","suit","summer","sun","sunny","sunset","super","supply","supreme",
...
"thing","this","thought","three","thrive","throw","thumb","thunder","ticket","tide","tiger","tilt","timber",
...
"worth","wrap","wreck","wrestle","wrist","write","wrong","yard","year","yellow","you","young","youth","zebra",
"zero","zone","zoo"
]
...
def load_wordlist():
    if len(words) != 2048:
        raise ValueError("Embedded wordlist must contain exactly 2048 words")
    return words
...
def validate_checksum(mnemonic, wordlist):
    idx = [wordlist.index(w) for w in mnemonic]
    bits = ''.join(format(i, '011b') for i in idx)
    entropy = bits[:256]
    checksum = bits[256:]
    entropy_bytes = bytes(int(entropy[i:i+8], 2) for i in range(0, 256, 8))
    hashbits = ''.join(format(b, '08b') for b in hashlib.sha256(entropy_bytes).digest())
    return checksum == hashbits[:8]

# ================= INTERACTIVE INPUT =================

def input_word_live(position, wordlist, previous_words):
    buffer = ""
    while True:
        print("\033[H\033[J", end="")
        if previous_words:
            print("Previous words:", " ".join(previous_words), "\n")
        print(f"Word {position}/24")
        matches = [w for w in wordlist if w.startswith(buffer)][:9] if buffer else []
        if matches:
            for i, w in enumerate(matches, 1):
                print(f"{i}. {w}")
        elif buffer:
            print("No suggestions")
        print("\nType letters | select number | Backspace | Ctrl+C to quit")
        print(f"> {buffer}", end="", flush=True)
        char = getch()
        if char in ('\r','\n'):
            if buffer in wordlist:
                return buffer
            if len(matches) == 1:
                return matches[0]
            continue
        if ord(char) in (8,127):
            buffer = buffer[:-1]
            continue
        if char.isdigit() and matches:
            idx_num = int(char)-1
            if 0 <= idx_num < len(matches):
                return matches[idx_num]
            continue
        if char.isalpha():
            buffer += char.lower()
            continue
...
def words_to_bits(words, wordlist):
    return ''.join(format(wordlist.index(w), '011b') for w in words)

def bits_to_words(bits, wordlist):
    return [wordlist[int(bits[i:i+11], 2)] for i in range(0, len(bits), 11)]

def mnemonic_to_entropy(words, wordlist):
    bits = words_to_bits(words, wordlist)
    entropy_bits = bits[:256]
    return int(entropy_bits,2).to_bytes(32, 'big')

def entropy_to_mnemonic(entropy_bytes, wordlist):
    cs = ''.join(format(b, '08b') for b in hashlib.sha256(entropy_bytes).digest())[:8]
    entropy_bits = ''.join(format(b, '08b') for b in entropy_bytes)
    full_bits = entropy_bits + cs
    return bits_to_words(full_bits, wordlist)

# ================= MAIN =================

def main():
    print("=== LIVE BIP39 INTERACTIVE ENTRY ===")
    print("Suggestions appear after first letter\n")

    wordlist = load_wordlist()
    args = sys.argv[1:]

    # ================= DICE MODE =================
    if len(args) == 1 and all(c in '123456' for c in args[0]):
        dice_string = args[0]
        if len(dice_string) < 98:
            print(f"Error: at least 98 dice rolls required, you provided {len(dice_string)}")
            sys.exit(1)
        dice_digits = ''.join(str(int(c)-1) for c in dice_string)
        dice_digits = dice_digits[-99:]  # Trim to fit 256-bit entropy
        num = int(dice_digits, 6)
        entropy = num.to_bytes(32, 'big')
        mnemonic = entropy_to_mnemonic(entropy, wordlist)
        print("Generated mnemonic from dice:")
        print(" ".join(mnemonic))
        passphrase = input("Enter passphrase to encrypt mnemonic: ").encode()
        hmac_digest = hashlib.sha256(entropy + passphrase).digest()[:4]
        encrypted = xor_bytes(entropy + hmac_digest, passphrase)
        b58 = base58.b58encode(encrypted).decode()
        b58 = b58.rjust(50, '1')  # pad to exactly 50 chars
        print("\nEncrypted Base58 string (50 chars):")
        print(b58)
        sys.exit(0)

    # ================= DECODE MODE =================
    if len(args) == 1 and len(args[0]) >= 48:
        b58_string = args[0]
        if len(b58_string) != 50:
            print(f"Error: Base58 string must be exactly 50 characters, got {len(b58_string)}")
            sys.exit(1)
        passphrase = input("Enter passphrase to decrypt mnemonic: ").encode()
        try:
            encrypted_bytes = base58.b58decode(b58_string)
            decrypted = xor_bytes(encrypted_bytes, passphrase)
            entropy = decrypted[:32]
            hmac_check = decrypted[32:36]
            if hmac_check != hashlib.sha256(entropy + passphrase).digest()[:4]:
                raise ValueError("Wrong passphrase or corrupted data")
            mnemonic = entropy_to_mnemonic(entropy, wordlist)
            print("Decoded mnemonic:")
            print(" ".join(mnemonic))
            if validate_checksum(mnemonic, wordlist):
                print("✅ BIP39 CHECKSUM OK")
            else:
    ...
    mnemonic = []
    for i in range(1,25):
        word = input_word_live(i, wordlist, mnemonic)
